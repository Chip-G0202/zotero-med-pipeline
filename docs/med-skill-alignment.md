...
- Before the single entrypoint, Agent layer must launch Zotero via Desktop Commander MCP tool `mcp__desktop_commander__.start_process` with fixed command `schtasks /Run /TN StartZoteroForCodexOnly`, wait 3000ms, set `ZOTERO_EXTERNAL_LAUNCHER=desktop_commander`, then run only `node --env-file=.env tools/run_zotero_literature_filter.mjs`.
