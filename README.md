...
| 变量 | 是否必填 | 说明 |
|---|---|---|
| `ZOTERO_API_KEY` | 必填 | Zotero MCP 插件的 API Key |
| `ZOTERO_LIBRARY_ID` | 必填 | Zotero 用户或群组库 ID |
| `ZOTERO_LIBRARY_TYPE` | 必填 | 个人库填 `user`，群组库填 `group` |
| `ZOTERO_MCP_URL` | 可选 | Zotero MCP 服务地址，默认 `http://127.0.0.1:23120/mcp` |
| `TITLE_TRANSLATION_API_KEY` | 可选 | 标题翻译 API Key（OpenAI 兼容接口） |
| `TITLE_TRANSLATION_ENDPOINT` | 可选 | 翻译 API 端点 |
| `TITLE_TRANSLATION_MODEL` | 可选 | 翻译模型名称 |
| `ZOTERO_EXE` | 可选 | Zotero 可执行文件路径（通常自动检测） |
| `PWSH_PATH` | 可选 | PowerShell 7 路径，默认 `pwsh` |
| `FORCE_RESEARCH_OS_RUN` | 可选 | 设为 `true` 强制立即运行，忽略 2 天间隔 |

**跨平台配置说明：**

- **Windows**：`ZOTERO_EXE` 默认候选为 `D:/Zotero/zotero.exe`，如安装在其他位置请自行修改
- **macOS**：通常可留空，系统会通过应用名启动（如 `open -a Zotero`）
- **Linux**：如 Zotero 在系统 PATH 中可留空，否则设置完整路径
- **ZOTERO_MCP_URL**：本地 MCP 服务地址，默认端口 `23120`，请根据实际插件配置修改
...
**字段说明：**
- `name`：期刊名称（自定义，用于显示）
- `url`：RSS 订阅地址（在期刊网站找到 RSS 图标，右键复制链接）
- `enabled`：`true` 启用，`false` 暂时禁用

**如何找到 RSS 地址？**
1. 打开期刊网站
2. 找到 RSS 图标（橙色方块）或搜索 "RSS"
3. 右键复制链接地址
4. 粘贴到 `url` 字段
...
```json
{
  ...
  "keyword_groups": {
    "required": [
      ...
      ["example-condition-1", "example-condition-2"]
    ],
    "optional": [
      ["example-method-1", "example-method-2"]
    ],
    "negative": [
      "example-exclude-term"
    ]
  }
}
```
...
**字段说明：**
- `days_back`：搜索最近多少天的文献（默认 7）
- `retmax`：最大结果数（默认 300）
- `query`：直接填写 PubMed 搜索表达式（如果填写，keyword_groups 会被忽略）
- `keyword_groups.required`：必须包含的关键词（组内 OR，组间 AND）
- `keyword_groups.optional`：可选关键词（存在则加分）
- `keyword_groups.negative`：排除的关键词

**如何编写 PubMed 搜索表达式？**
1. 前往 [PubMed](https://pubmed.ncbi.nlm.nih.gov/)
2. 在搜索框输入关键词测试
3. 找到满意表达式后，点击 "Advanced" 查看完整表达式
4. 复制到 `query` 字段

#### 4.3 筛选标准 (`screening_standards.md`)

这是你的核心筛选规则文件，管线会根据这些规则将文献分为 A/B/C/D 四级。

**文件结构：**
- `## 优先关注`：你最关注的研究方向（每行以 `*` 开头）
- `## 相对降权`：你希望降低优先级的研究类型
- `## 严格排除`：直接排除的规则
- `## 不确定边界`：反馈冲突的案例
...
* 优先关注与当前课题直接相关的机制研究、实验验证研究
* 优先关注随机对照试验和高质量队列研究
* 优先关注具有明确生物标志物或分子机制的研究
...
> 💡 **提示**：首次运行时如果 `screening_standards.md` 不存在，管线会自动创建默认版本。建议手动修改以适配你的研究方向。
...
- `journal_whitelist`：你关注的期刊

**其他字段（通常无需修改）：**
- `weights`：各关键词维度的权重
- `thresholds`：A/B/C/D 各级的分数阈值
...
**可调参数：**
- `model`：翻译模型名称（填入你的模型）
- `batch_size`：每批处理多少条（越大越快，但 API 压力越大）
- `fallback_to_english`：翻译失败时是否使用英文标题（建议保持 `true`）
...
1. 打开 Codex Desktop
2. 将项目文件夹拖入 Codex 工作区
3. 告诉 Codex：**"运行医学文献管线"**

Codex 会自动按顺序执行所有阶段，在 `research_os/文献评价/` 下生成 `隔日报.xlsx`。

### Step 6: 配置自动化

管线内置 2 天间隔门禁（默认每 2 天运行一次）。你可以通过 Codex 的 Automation 功能设置自动任务。
...
Codex 会调用 `automation_update` 工具创建定时任务。

**方法 2：手动设置自动化**

在 Codex 的 Automation 面板：
1. 点击 "Create Automation"
2. 设置名称（如 "Medical Literature Pipeline"）
3. 设置时间表（如 `FREQ=DAILY;INTERVAL=2` 表示每 2 天）
4. 设置工作目录为项目根目录
5. 设置提示词为 `"运行医学文献管线"`

**关于间隔门禁：**
- 默认每 2 天运行一次（`RESEARCH_OS_RUN_INTERVAL_DAYS=2`）
- 可在 `.env` 中设置 `FORCE_RESEARCH_OS_RUN=true` 强制立即运行
- 间隔逻辑使用 `Asia/Shanghai` 时区，15:00 计划槽语义
...
3. **评分方式**：
   - 鼠标点击星星：直接设置 1-5 星
   - 快捷键：选中文献后，按数字键 `1`-`5` 设置星级，按 `0` 清除评分
...
- **星标迁移功能** — 支持通过 Zotero Style 插件的评分功能，自动将高星文献迁移到「值得精读」收藏夹
