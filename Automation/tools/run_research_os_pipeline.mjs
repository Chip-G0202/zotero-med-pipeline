...
import { LABELS, TRIAGE_VERSION, classifyItem, loadScreeningStandards, summarizeGradeCounts, deriveSemanticGradeFromStandards, synthesizeFinalGrade } from "./lib/triage_policy.mjs";
...
import { ensureWithStrongRecovery, restartWorkflowProcess } from "./lib/workflow_startup_ready.mjs";
...
import { buildOpenAlexUrl, normalizeOpenAlexItems } from "./lib/openalex_adapter.mjs";
import { applyJournalQualityGate, buildJournalQualityConfig } from "./lib/easyscholar_journal_quality_gate.mjs";
...
async function fetchPubMed(externalCfg) {
  const cfg = externalCfg || loadPubMedPmcSearchConfig({ root: ROOT, now: new Date() });
  const items = [];
  const failed = [];
...
async function fetchOpenAlex(cfg) {
  try {
    const url = buildOpenAlexUrl(cfg);
    const json = await fetchJson(url, 30000);
    const works = json?.results || [];
    const items = normalizeOpenAlexItems(works);
    return { items, failed: [], config: cfg };
  } catch (error) {
    return { items: [], failed: [{ source: "openalex", error: String(error.message || error) }], config: cfg };
  }
}
...
export async function runResearchOsPipeline({ argv = process.argv } = {}) {
  ...
  if (!runDue && !forceRun) {
    const skipReport = {
      ...
      forceRun,
      month_dir: monthDir,
      review_day_dir: reviewDayDirName,
  ...
  const pubmedCfg = loadPubMedPmcSearchConfig({ root: ROOT, now: new Date() });
  const dbDatabases = pubmedCfg.databases.filter((d) => d === "pubmed" || d === "pmc");
  const openalexDatabases = pubmedCfg.databases.filter((d) => d === "openalex");
  ...
  if (dbDatabases.length > 0) {
    const dbResult = await fetchPubMed(pubmedCfg).catch((e) => ({ items: [], failed: [{ source: "pubmed", error: String(e.message || e) }] }));
    dbItems = dbResult.items || [];
    dbFailed = dbResult.failed || [];
  }

  if (openalexDatabases.length > 0) {
    const oalexResult = await fetchOpenAlex(pubmedCfg).catch((e) => ({ items: [], failed: [{ source: "openalex", error: String(e.message || e) }] }));
    openalexItems = oalexResult.items || [];
    openalexFailed = oalexResult.failed || [];
  }

  const db = { items: [...dbItems, ...openalexItems], failed: [...dbFailed, ...openalexFailed], config: pubmedCfg };
  ...
  report.counts.openalex_raw = openalexItems.length;
  ...
  report.steps.med_entry_parallel = {
    ...
    openalex_databases: openalexDatabases,
  };
  ...
  // Uses strong recovery: if first probe fails, kill ollama process → restart → re-probe.
  report.steps.ollama = { ok: false, skipped: true };
  if (semanticAdapter.enabled) {
    try {
      const ollamaRecovery = await ensureWithStrongRecovery({
        target: "ollama",
        ensureReady: () => ensureOllamaReady(),
        restartProcess: restartWorkflowProcess,
      });
      report.steps.ollama = {
        ok: ollamaRecovery.ready,
        started_now: ollamaRecovery.result?.started_now ?? false,
        recovered: ollamaRecovery.recovered,
        attempts: ollamaRecovery.attempts,
        recovery: ollamaRecovery.recovery,
        models: ollamaRecovery.result?.models ?? [],
      };
      if (!ollamaRecovery.ready) {
  ...
  if (semanticAdapter.enabled && report.steps.ollama?.ok) {
    ...
    // Load workflow rules for standards-based grading
    const workflowRules = loadWorkflowRules();
    const researchFocus = workflowRules?.config?.triage?.research_focus || {};
    ...
    // ── Process batch results with standards-based grading ──
    for (let i = 0; i < batchResults.length; i++) {
      const searchResult = batchResults[i];
      const items = batchSamples[i]._items;
      const ruleGrade = items[0].rule_grade;

      const derived = deriveSemanticGradeFromStandards({
        item: items[0],
        ruleGrade,
        searchResults: searchResult.ok ? searchResult.results : [],
        researchFocus,
      });
      ...
      for (const item of items) {
        ...
        item.semantic_confidence = derived.semanticConfidence;
        item.semantic_source = derived.semanticSource;
    ...
    // Track semantic source distribution
    const sourceCounts = {};
    for (const item of triagedAll) {
      const src = item.semantic_source || "none";
      sourceCounts[src] = (sourceCounts[src] || 0) + 1;
    }
    semanticGradingReport.semantic_source_counts = sourceCounts;
  ...
  report.steps.med_monthly_synthesis = { ok: false, deferred_until: "finalize_research_os_exports" };
