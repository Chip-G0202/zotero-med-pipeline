...
import { LABELS, normalizeGradeLetter } from "./lib/triage_policy.mjs";
...
/**
 * Resolve the effective grade name for Zotero collection routing.
 * Priority: final_grade (semantic-adjusted) > grade (rule-based) > grade_label (legacy).
 * Returns full label like "A课题相关", defaults to LABELS.C if unresolvable.
 */
function resolveGradeName(item) {
  const letter = normalizeGradeLetter(item?.final_grade)
    || normalizeGradeLetter(item?.grade)
    || "";
  if (letter) return LABELS[letter] || LABELS.C;
  return GRADE_COLLECTIONS[item?.grade_label] || LABELS.C;
}
...
// especially under concurrent load. The asciiEscapeJson() serializer below is the primary fix;
// the mutex is a secondary safeguard against SQLite lock contention.
...
async function mcpToolCall(name, args, id) {
  ...
  // Chain onto the global mutex so only one MCP request is in-flight at a time.
    ...
    _mcpMutexChain = _mcpMutexChain.then(async () => {
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          ...
          // Network-level error (ECONNREFUSED, etc.) — also retry
...
function buildExtra(it) {
  return safeStr([
    ...
    `final_grade: ${it.final_grade || ""}`,
    `effective_grade: ${resolveGradeName(it)}`,
  ].filter(Boolean).join("\n"));
}
...
async function createItem(it, i) {
  const fields = {
    ...
    publicationTitle: cleanJournalName(it.journal) || await inferJournalFromUrl(it.url, it.feed_url),
    abstractNote: safeStr(it.abstract),
    extra: buildExtra(it),
  };
  const itemType = "journalArticle";
  const tags = ["research-os", "自动入库", resolveGradeName(it), it.source_channel || ""]
...
export async function migrateRatedItems({ rootKey, worthyKey, now, mcpToolCall, starMigrationConfig, collectionGuard = null, collectionScopeBlocks = null }) {
  ...
  const dateNodes = collectRecentDateCollectionNodes(tree, now, stats.window_days);
...
export async function runMcpBulkWriteback({ argv = process.argv } = {}) {
  ...
  const zoteroMonthName = monthLabel(TODAY);
  const zoteroDayName = monthlyDayLabel(TODAY);
  ...
  const monthKey = await ensureChildCollection(root.key, zoteroMonthName, 50, { collectionGuard, collectionScopeBlocks });
  collectionGuard = await buildCollectionGuard(root.key);
  const dateKey = await ensureChildCollection(monthKey, zoteroDayName, 54, { collectionGuard, collectionScopeBlocks });
  collectionGuard = await buildCollectionGuard(root.key);
    ...
    const running = (async () => {
      ...
      if (!itemKey) {
        for (let attempt = 0; attempt <= writebackRetryLimit; attempt++) {
          try {
            if (attempt > 0) await wait(1000 * attempt); // backoff for transient MCP errors (e.g. SQLite lock)
            itemKey = await createItem(it, i);
            break;
          } catch (createError) {
            const isTransient = /-32700|Parse error|timeout|database busy|lock conflict/i.test(String(createError?.message || createError));
            if (attempt >= writebackRetryLimit && !isTransient) throw createError;
            if (attempt >= writebackRetryLimit + 2) throw createError; // hard cap for transient retries
            retryCount += 1;
