...
export async function runZoteroLiteratureFilter({
  ...
  // Verify Stage1 produced required artifacts. Stage1 may exit code 0 but skip
  // internally (e.g. its own interval gate), leaving writeback_ready_items.json
  // and desktop_daily_review_source.json absent. Proceeding would cause Stage2
  // to read stale or missing data.
  {
    const requiredArtifacts = [
      `${config.pipelineDir}/writeback_ready_items.json`,
      `${config.pipelineDir}/desktop_daily_review_source.json`,
    ];
    let stage1ArtifactsMissing = false;
    for (const ap of requiredArtifacts) {
      const st = await statArtifact(ap);
      if (!st.exists || st.mtimeMs < Date.parse(stages.find((s) => s.name === "stage1")?.startedAt || 0)) {
        stage1ArtifactsMissing = true;
        break;
      }
    }
    if (stage1ArtifactsMissing) {
      stages.push(skippedStage(stageDefs.mcpReady.name, stageDefs.mcpReady.scriptPath, "stage1_artifacts_missing", clock));
      stages.push(skippedStage(stageDefs.stage2.name, stageDefs.stage2.scriptPath, "stage1_artifacts_missing", clock));
      stages.push(skippedStage(stageDefs.stage3.name, stageDefs.stage3.scriptPath, "stage1_artifacts_missing", clock));
      stages.push(skippedStage(stageDefs.stage4.name, stageDefs.stage4.scriptPath, "stage1_artifacts_missing", clock));
      const report = buildOrchestratorReport({
        status: "completed_stage1_only",
        runContext,
        finishedAt: iso(clock()),
        stages,
        artifacts,
        extra: { startup, stage1ArtifactsMissing: true, reason: "stage1_skipped_due_to_internal_interval_gate" },
      });
      await writeReport(report);
      return report;
    }
