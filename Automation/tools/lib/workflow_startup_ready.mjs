...
export async function restartWorkflowProcess(target, { platform = process.platform, spawnSyncFn } = {}) {
  ...
  if (family === "windows") {
    // Windows sandbox: taskkill/wmic are not spawnable.
    // Log intent but do not attempt system binary calls.
    commands.push({
      method: `restart_${normalizedTarget}_skipped_windows_sandbox`,
      command: `skipped: ${normalizedTarget} restart not available in sandbox`,
      success: false,
      exitCode: null,
      stdout: "",
      stderr: "skipped: system binary spawning not available in sandbox environment",
