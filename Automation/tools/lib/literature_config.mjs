...
export function loadPubMedPmcSearchConfig({ root, now = new Date() } = {}) {
  ...
  const fallback = {
    ...
    sort: "date",
    datetype: "pdat",
    query: "(EXPOSURE_EXAMPLE OR EXPOSURE_EXAMPLE OR EXPOSURE_EXAMPLE OR exposure OR exposure) AND (BIOOUTCOME_EXAMPLE OR CELLTYPE_EXAMPLE OR INFLAMMATION_EXAMPLE OR brain)",
  };
...
export function loadWorkflowRules({ root } = {}) {
  ...
  const fallback = {
    triage: {
      ...
      terms: {
        exposure: ["environmental_exposure", "exposure", "EXPOSURE_EXAMPLE", "EXPOSURE_EXAMPLE", "EXPOSURE_EXAMPLE", "exposure", "toxic"],
        core_topic: ["CELLTYPE_EXAMPLE", "neuroinflamm", "brain", "cognitive", "mitochond", "synap", "neurotox"],
        mechanism: ["pathway", "mechanism", "axis", "oxidative", "omics", "signaling", "model"],
      },
      journal_whitelist: ["nature", "nature neuroscience", "nature reviews neuroscience", "science", "science advances", "cell", "cell reports", "neuron", "environmental health perspectives", "environmental science & technology", "environment international", "environmental environmental_exposure", "journal of INFLAMMATION_EXAMPLE"],
      weights: { exposure: 1.6, core_topic: 1.5, mechanism: 0.7, journal_quality: 1.2, feedback_positive: 0.6, feedback_negative: -1.0 },
      thresholds: { A_score: 6.0, A_min_exposure_hits: 2, A_min_core_hits: 2, B_score: 3.4, C_score: 1.4, B_uncertain_below: 4.2, C_uncertain_below: 2.3 },
