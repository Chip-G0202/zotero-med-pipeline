...
export function buildCandidateFeedbackFiles(now, { reviewRoot, desktopRoot, projectRoot, researchRoot, lookbackDays = 7 }) {
  ...
  for (let d = 1; d <= lookbackDays; d++) {
    ...
    const dayFiles = [
      path.join(reviewRoot, month, monthDay, "隔日报.xlsx"),
      path.join(reviewRoot, month, monthDay, "daily_review.xlsx"),
      path.join(desktopRoot, month, monthDay, "隔日报.xlsx"),
      path.join(desktopRoot, month, monthDay, "daily_review.xlsx"),
      path.join(legacyProjectRoot, "文献评价", month, monthDay, "隔日报.xlsx"),
      path.join(legacyProjectRoot, "文献评价", month, monthDay, "daily_review.xlsx"),
