...
function isEnvironmentalPollutantFateStudy(text) {
  const pollutantFateTerms = [
    ...
    "degradation by dbd", "degradation of pf", "degradation of microplastic",
    ...
    "pfas removal", "pfas degradation", "heavy metal removal",
    // 污染物转移
    ...
    // PFAS / 新污染物治理
    "pfas degradation", "pfas treatment", "forever chemical",
    "dbd combined", "dielectric barrier discharge",
    // 环境微生物学（非宿主相关）
    "glacier microbiome", "glacier microbial", "resistome",
    "antimicrobial resistance dissemination", "antibiotic resistance gene",
    "environmental microbiome", "soil microbiome",
    "aquatic microbiome", "sediment microbiome",
  ...
  const biomedicalIndicators = [
    ...
    "patient", "clinical", "in vivo mice", "in vivo rat",
  ];
  
  const hasEnvironmentalFocus = pollutantFateTerms.some((term) => text.includes(term));
...
function isPureEngineeringOrMaterialsStudy(text) {
  const engineeringTerms = [
    ...
    // 工程 / 声学 / 计算
    "neuromorphic computing", "acoustic synapse", "acoustic metamaterial",
  ...
  const biomedicalIndicators = [
    ...
    "disease model", "animal model", "in vivo", "in vitro",
    "apoptosis", "proliferation", "cytotoxicity", "cell viability",
  ];
...
function isOutOfScopeTopic(text) {
  ...
  const relevantContextIndicators = [
    ...
    "animal model", "in vivo", "in vitro", "experimental",
  ];
