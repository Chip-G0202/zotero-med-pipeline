...
- Feedback item actions are enabled by default: `drop` items move to `文献池/待删除`, and `upgrade` / `downgrade` items update the corresponding grade collections. Set `APPLY_FEEDBACK_ITEM_ACTIONS=false` only when you want a plan-only run.
...
- `schtasks /Run /TN StartZoteroForCodexOnly` is legacy/manual only. Desktop Commander may be used as the scheduled/background prewarm process channel only for `node --env-file=.env tools/start_workflow_dependencies.mjs`.
