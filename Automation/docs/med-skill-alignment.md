...
- Before the single entrypoint, the Agent layer must set workflow control environment variables only and must not pre-launch Zotero via `StartZoteroForCodexOnly`.
- The single entrypoint owns the primary workflow and primary startup path: interval gate first, then repository bootstrap/local fallback only when the run is due, then Stage 1 -> MCP ready -> Stage 2 -> Stage 3 -> Stage 4.
- Default scheduled/background automation must not set `ZOTERO_EXTERNAL_LAUNCHER=desktop_commander`; Desktop Commander may be used only as the third fallback when the first entrypoint run reports `startup.failureClass=process_permission_denied`, by running `node --env-file=.env tools/start_workflow_dependencies.mjs` from the project root and then rerunning the single entrypoint exactly once.
- Startup success is functional: Zotero MCP `get_collections` succeeds and Ollama `GET /api/tags` succeeds. Process presence is diagnostic only.
- In `desktop_commander` external-launcher mode, Stage2/Stage3 helper still performs MCP readiness check only and must not execute local fallback launch paths.
...
- Stage 1 default-enabled feedback item actions must report `feedback_item_actions_default_enabled=true`; docx rule rewrite must report `manual_standard_evaluation_default_enabled=true`, key configured status, and direct/fallback key source without exposing secret values.
...
- Manual review files live under `research_os/文献评价`: `隔日报.xlsx`, `run_log.xlsx`, `双周报-*.docx`, `review_queue.xlsx`, `contradiction_log.xlsx`, `screening_standards.md`, and `screening_preferences.xlsx`.
