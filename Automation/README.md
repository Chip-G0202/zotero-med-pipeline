...
| ZOTERO_EXE | 平台默认候选：Windows `D:/Zotero/zotero.exe`，macOS `/Applications/Zotero.app/Contents/MacOS/zotero` 或 `/Applications/Zotero.app`，其他平台默认使用 PATH 中的 `zotero` |
| PWSH_PATH | pwsh |
...
| ZOTERO_EXE | 可选 | Zotero 可执行文件路径。Windows 默认候选：`D:/Zotero/zotero.exe`；macOS 默认候选：`/Applications/Zotero.app/Contents/MacOS/zotero` 或 `/Applications/Zotero.app`；其他平台默认使用 PATH 中的 `zotero` |
...
- **最低运行环境**：`Node.js >= 18`，`pwsh >= 7.0.0`。
- 当前仓库已提交 `package.json` 和 `package-lock.json`；新环境先运行 `npm install`。
- 本地脚本统一使用 Node 直接运行，例如：
  - `node tools/run_zotero_literature_filter.mjs`
  - `node tests/literature_config.test.mjs`
- **环境变量**：先复制 `.env.example .env`，再按项目说明填写 `TITLE_TRANSLATION_API_KEY`、`ZOTERO_MCP_URL`、`ZOTERO_EXE` 等变量。
- **默认验证命令**：`node --test tests/*.test.mjs`
  - 如果环境里缺少 Node 或测试依赖读取失败，应记录为环境问题，不要当作业务逻辑失败。
- **当前可复现性缺口**：
  - 已提交 lockfile；
  - 没有统一 CI workflow；
  - 没有 `engines` / `.node-version` 等固定 Node 版本文件。
...
- **Writeback Pool Dedup** — Exact normalized dedupe for DOI/PMID/PMCID/arXiv/title before Zotero writeback; checks `文献池`, `文献池/待删除`, and `值得精读`, with stronger title normalization for Unicode/punctuation/spacing variants
...
- **Screening Standards Management** — Auto-generated screening_standards.md, rule suggestion table, change tracking
- **Enhanced Triage Engine** — Hard exclusion rules, multi-keyword dimensions, journal whitelist
- **Writeback Pool Dedup** — Avoid duplicate entries in Zotero; current-day writeback checks `文献池`, `文献池/待删除`, and `值得精读` before creation
...
- **Bi-weekly Synthesis Reports** — Every two weeks generates a `双周报-*.docx` summary report for trend review
...
|---|---|
| `ZOTERO_MCP_URL` | `http://127.0.0.1:23120/mcp` |
| `ZOTERO_EXE` | Platform default candidate; Windows `D:/Zotero/zotero.exe`, macOS `/Applications/Zotero.app/Contents/MacOS/zotero` or `/Applications/Zotero.app`, other platforms `zotero` in `PATH` |
| `PWSH_PATH` | `pwsh` |
...
ZOTERO_MCP_URL=http://127.0.0.1:23120/mcp
# Windows example: D:/Zotero/zotero.exe
# macOS example: /Applications/Zotero.app/Contents/MacOS/zotero
ZOTERO_EXE=
...
| `ZOTERO_EXE` | Optional | Zotero executable path. Windows default candidate: `D:/Zotero/zotero.exe`; macOS default candidate: `/Applications/Zotero.app/Contents/MacOS/zotero` or `/Applications/Zotero.app`; Linux/other platforms default to `zotero` in `PATH` |
...
## 🧑‍💻 Developer reproducibility checklist (added)

- Minimum runtime: **Node.js >= 18** and **PowerShell 7 (pwsh) >= 7.0.0**.
- The repository has committed `package.json` and `package-lock.json`.
- Local scripts are executed directly with the Node binary, for example `node tools/run_zotero_literature_filter.mjs` or `node tests/literature_config.test.mjs`.
- Environment setup: `copy .env.example .env` (Windows) or `cp .env.example .env` (macOS/Linux), then configure `TITLE_TRANSLATION_API_KEY`, `ZOTERO_MCP_URL`, `ZOTERO_EXE`, and other variables as documented.
- Local validation command: `node --test tests/*.test.mjs`.
- Current reproducibility gaps: no committed CI workflow and no pinned Node version file such as `.node-version`.

---

## 🍎 macOS setup notes (added)

macOS is now a supported setup target, but the recommended onboarding sequence differs from Windows:

1. Install dependencies:
   - Node.js 18+
   - Zotero 7.x
   - Zotero MCP plugin
   - Optionally `pwsh` via Homebrew: `brew install --cask powershell`
...
3. Edit `.env` for your platform:
   ...
   ```ini
   ZOTERO_MCP_URL=http://127.0.0.1:23120/mcp
   ZOTERO_EXE=/Applications/Zotero.app/Contents/MacOS/zotero
   PWSH_PATH=/opt/homebrew/bin/pwsh
   ```

4. Stage behavior on macOS:
   - **Stage 1 can run without Zotero MCP.**
   - **Stage 2 and Stage 3 are skipped only when Zotero MCP is unavailable or when you explicitly run Stage 1-only mode.**
   - The helper `tools/lib/ensure_zotero_mcp_ready.mjs` now uses platform-aware Zotero process detection and macOS launch paths instead of Windows-only assumptions.
   - The `ZOTERO_EXTERNAL_LAUNCHER=desktop_commander` path remains Windows-oriented because the startup task command is `schtasks /Run /TN StartZoteroForCodexOnly`; on macOS, do not use this external launcher mode. Use local process detection and the default Zotero path instead.
   - **Full pipeline parity on macOS requires Zotero.app + Zotero MCP plugin to be installed and listening at `ZOTERO_MCP_URL`.**

5. Stage 1-only smoke test:
   ```bash
   node --env-file=.env tools/run_zotero_literature_filter.mjs --stage1-only
   ```

6. Full pipeline readiness / parity test:
   ```bash
   node --env-file=.env tools/check_zotero_mcp_ready.mjs
   ```

7. Full pipeline run (macOS):
   ```bash
   node --env-file=.env tools/run_zotero_literature_filter.mjs
   ```
   **macOS 目标是 full pipeline parity**：当 Zotero.app + Zotero MCP 正常运行时，Stage 1→2→3→4 全流程应与 Windows 等价。
   - Stage 1-only 是 no-MCP 冒烟测试，不代表完整成功。
   - Stage 2/3 仅在 MCP unavailable 或显式 `--stage1-only` 时跳过。
   - MCP 不可用时 orchestrator 报告 `degraded_due_to_mcp_unavailable`，不会误报成功。
   - 不要设置 `ZOTERO_EXTERNAL_LAUNCHER=desktop_commander`（该路径仅 Windows 可用）。
...
Check three things:
1. PowerShell version is ≥ 7.0 (`pwsh --version`) — required for full pipeline; Stage 1-only can run without pwsh
2. `.env` file is properly configured
3. Zotero and Zotero MCP plugin are running

On macOS, additionally verify:
- Zotero.app is installed in `/Applications/`
- Zotero MCP plugin is loaded and listening at `ZOTERO_MCP_URL` (default: `http://127.0.0.1:23120/mcp`)
- Do NOT set `ZOTERO_EXTERNAL_LAUNCHER=desktop_commander` (Windows-only)
...
- `双周报-*.docx`: Bi-weekly synthesis report
