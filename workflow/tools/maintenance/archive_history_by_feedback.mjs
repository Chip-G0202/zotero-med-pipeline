...
import { normalizeDoi as normalizeDoiValue } from "../lib/doi_normalization.mjs";
  ...
  for (const file of files) {
    ...
    items.forEach((item, index) => {
      ...
      records.push({
        ...
        current_level: normalizeLevel(item.final_grade || item.grade_label || item["推荐等级"] || item.grade),
        item,
      });
    });
  }
...
export async function scanFeedbackRows(reviewRoot) {
  const files = await walkFiles(reviewRoot, (_full, name) => name === "周报.xlsx" || name === "隔日报.xlsx" || name === "daily_review.xlsx");
  const rows = [];
  for (const file of files) {
    let sheets;
    ...
    for (const row of rowsToObjects(sheetRows)) {
      ...
      const explicitLevel = normalizeLevel(pick(data, ["最终等级", "推荐等级", "等级", "grade", "Grade"]));
