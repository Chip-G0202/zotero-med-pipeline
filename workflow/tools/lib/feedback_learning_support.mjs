...
import { readScreeningStandardsFileSync } from "./screening_standards_paths.mjs";
...
export function runFeedbackLearningDiagnostic(now, { reviewRoot, desktopRoot, projectRoot, researchRoot, writeReportPath = null, lookbackDays = 7 } = {}) {
  ...
  const result = {
    ...
    samples_preview: [],
    preference_learning: { would_update_preference: false, positive_samples: 0, negative_samples: 0, keep_samples: 0, upgrade_samples: 0, downgrade_samples: 0, drop_samples: 0, positive_weight: 0, negative_weight: 0, ambiguous_samples: 0, ignored_samples: 0, blockers: [...reader.blockers] },
    recommendations: [],
    learning_payload: { rows_used: 0, weighted_rows_used: 0, hardPositiveTerms: [], hardNegativeTerms: [], signals: [], meta_preference_signals: [], screening_standards: screeningStandards },
  };
  if (reader.workbook_unreadable) {
    ...
    for (const s of reader.learning_signals) {
      ...
      if (s.feedback === "keep") result.preference_learning.keep_samples += 1;
      if (s.feedback === "upgrade") result.preference_learning.upgrade_samples += 1;
      if (s.feedback === "downgrade") result.preference_learning.downgrade_samples += 1;
      if (s.feedback === "drop") result.preference_learning.drop_samples += 1;
      if (s.feedback === "keep" || s.feedback === "upgrade") {
        result.preference_learning.positive_samples += 1;
        result.preference_learning.positive_weight += weight;
      } else if (s.feedback === "drop" || s.feedback === "downgrade") {
        result.preference_learning.negative_samples += 1;
        result.preference_learning.negative_weight += weight;
      }
      ...
      if (s.feedback === "keep" || s.feedback === "upgrade") {
        for (const t of tokenize(`${s.title_context} ${s.english_title} ${s.comment}`)) pos[t] = (pos[t] || 0) + weight;
      }
      if (s.feedback === "drop" || s.feedback === "downgrade") {
        for (const t of tokenize(`${s.title_context} ${s.english_title} ${s.comment}`)) neg[t] = (neg[t] || 0) + weight;
      }
    }
    ...
    result.learning_payload.weighted_rows_used = Number((result.preference_learning.positive_weight + result.preference_learning.negative_weight).toFixed(2));
