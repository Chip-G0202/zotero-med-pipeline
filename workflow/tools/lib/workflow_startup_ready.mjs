import { ensureZoteroBackendReady } from "./ensure_zotero_backend_ready.mjs";
...
/**
 * Restart a workflow process by killing it, so that a subsequent
 * ensureReady() can launch a fresh instance.
 *
 * Windows: uses taskkill /f /im via cmd /c (may fail in restricted sandbox).
 * macOS:  uses pkill -x.
 * Linux:  uses pkill -f.
 *
 * Returns gracefully with killed=false when kill is not possible,
 * allowing the caller to proceed with a fresh ensureReady() attempt.
 */
export async function restartWorkflowProcess(target, { platform = process.platform, spawnSyncFn } = {}) {
  const normalizedTarget = String(target || "").trim().toLowerCase();
  if (normalizedTarget !== "zotero") {
    return {
      target: normalizedTarget,
      killed: false,
      commands: [],
      skipped: true,
      reason: "unsupported_restart_target",
    };
  }
  const family = platformName(platform);
  const commands = [];

  // Try to kill the target process. Graceful failure is expected in sandbox.
  if (family === "windows") {
    // Windows: taskkill /f /im via cmd /c. May fail in restricted sandbox.
    const exeName = "zotero.exe";
    try {
      const syncFn = spawnSyncFn || (await import("node:child_process")).spawnSync;
      const result = syncFn("cmd", ["/c", "taskkill", "/f", "/im", exeName], { encoding: "utf8" });
      const output = `${result?.stdout || ""}\n${result?.stderr || ""}`;
      const success = result?.status === 0 || /not found|没有找到/i.test(output);
      commands.push({
...
export async function ensureWorkflowStartupReady({
  ...
  const ensureZoteroReady = dependencies.ensureZoteroBackendReady || ensureZoteroBackendReady;
  const restartProcess = dependencies.restartProcess || restartWorkflowProcess;
  const zoteroReadyOptions = {
    retries: parsePositiveInt(process.env.WORKFLOW_STARTUP_ZOTERO_BACKEND_RETRIES || process.env.WORKFLOW_STARTUP_ZOTERO_MCP_RETRIES, DEFAULT_ZOTERO_STARTUP_RETRIES),
    intervalMs: parsePositiveInt(process.env.WORKFLOW_STARTUP_ZOTERO_BACKEND_INTERVAL_MS || process.env.WORKFLOW_STARTUP_ZOTERO_MCP_INTERVAL_MS, DEFAULT_ZOTERO_STARTUP_INTERVAL_MS),
  ...
  if (!diagnostics.zotero.ready) {
    ...
    throw createStartupError("WORKFLOW_STARTUP_FAILED", "Zotero backend was not ready after startup recovery", diagnostics);
  }
