...
export const REVIEW_REPORT_LABEL = "周报";
export const REVIEW_WORKBOOK_FILE_NAME = "周报.xlsx";
...
export function isLastDueRunOfMonth(date = new Date(), intervalDays = 7) {
  const current = new Date(date);
  const interval = Math.max(1, Number(intervalDays || 7));
